...
export default function Dashboard({ stats, recentNews, recentComments }) {
  ...
  const statItems = [
    ...
    {
      name: 'Ср. просмотров новостей',
      stat: formatNumber(stats?.averageViews),
      icon: (
        <svg className="h-6 w-6 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <>
      <div className="mb-8">
        ...
        <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {statItems.map((item) => (
            <div key={item.name} className="bg-white overflow-hidden shadow rounded-2xl">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {/* Последние новости */}
        <div className="bg-white/90 backdrop-blur-sm shadow-sm rounded-2xl border border-gray-100/50">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <Link
                          ...
                          className="inline-flex items-center px-3 py-1 border border-transparent text-sm leading-5 font-medium rounded-xl text-white bg-blue-600 hover:bg-blue-500 focus:outline-none focus:border-blue-700 focus:shadow-outline-blue active:bg-blue-700 transition ease-in-out duration-150"
                        >
                          Изменить
                        </Link>
                      </div>
