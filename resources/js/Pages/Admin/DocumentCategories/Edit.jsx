...
export default function Edit({ auth, category }) {
  const { data, setData, patch, processing, errors } = useForm({
    ...
    accordion_id: category.accordion_id,
    page: category.page || '',
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    patch(route('admin.document-categories.update', category.id));
  };

  return (
    <AuthenticatedLayout
      user={auth.user}
      header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Редактирование категории документов</h2>}
    >
      <Head title="Редактирование категории документов" />
