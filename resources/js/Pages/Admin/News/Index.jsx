        ...
        <AdminLayout title={meta?.title}>
            <div className="min-h-screen bg-gray-50/50 pb-12 pt-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    {/* Header Section */}
                    <div className="mb-8 bg-white/90 backdrop-blur rounded-2xl p-6 shadow-sm border border-gray-100/80 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                        ...
                        <div className="flex flex-wrap gap-3 w-full sm:w-auto">
                            <a
                                href={route('admin.news.export-views')}
                                className="flex-1 sm:flex-none justify-center bg-white text-gray-700 border border-gray-200 px-4 py-2.5 rounded-xl hover:bg-gray-50/50 transition-colors inline-flex items-center gap-2 font-medium shadow-sm hover:shadow"
                                download
                            >
                            ...
                            <Link
                                href={createRoute}
                                className="flex-1 sm:flex-none justify-center bg-blue-600 text-white px-5 py-2.5 rounded-xl hover:bg-blue-700 hover:shadow-lg transition-all font-medium inline-flex items-center gap-2"
                            >
                    ...
                    {/* Фильтры */}
                    <div className="bg-white/80 backdrop-blur shadow-sm border border-gray-100/80 rounded-2xl p-6 mb-8">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full border-gray-200 rounded-xl shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                />
                            </div>
                            ...
                            {/* Кнопки */}
                            <div className="flex flex-col sm:flex-row items-end gap-3 mt-4 md:mt-0">
                                <button
                                    onClick={applyFilters}
                                    className="w-full sm:w-auto bg-blue-600 text-white px-5 py-2 rounded-xl hover:bg-blue-700 hover:shadow-md transition-all font-medium"
                                >
                                    Применить
                                </button>
                                <button
                                    onClick={resetFilters}
                                    className="w-full sm:w-auto bg-white border border-gray-200 text-gray-700 px-5 py-2 rounded-xl hover:bg-gray-50/50 hover:shadow-sm transition-all font-medium"
                                >
                                    Сбросить
                                </button>
                            </div>
                                    ...
                                    <div className="flex flex-1 flex-col gap-4 p-5">
                                        ...
                                        <div className="mt-auto flex flex-wrap gap-2 pt-4 border-t border-gray-100">
                                            <Link
                                                ...
                                                className="inline-flex flex-1 items-center justify-center rounded-xl bg-blue-50/80 px-4 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-100 hover:shadow-sm"
                                            >
                                                Редак.
                                            </Link>
                                            <button
                                                ...
                                                className={`inline-flex flex-1 items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold transition hover:shadow-sm ${item.status === 'published' ? 'bg-orange-50/80 text-orange-700 hover:bg-orange-100' : 'bg-green-50/80 text-green-700 hover:bg-green-100'}`}
                                            >
                                                {item.status === 'published' ? 'Скрыть' : 'Публ.'}
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => handleDelete(item)}
                                                className="inline-flex items-center justify-center rounded-xl bg-red-50/80 px-4 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-100 hover:shadow-sm"
                                                title="Удалить"
                                            >
                    ...
                    {news.links && news.links.length > 3 && (
                        <div className="mt-10 mb-6 rounded-2xl border border-gray-100/50 bg-white/80 backdrop-blur shadow-sm px-4 py-4 sm:px-6">
