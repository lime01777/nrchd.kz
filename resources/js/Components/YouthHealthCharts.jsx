...
export const YouthTrainingChart = ({ t }) => {
    ...
    return <Bar data={data} options={{ indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Уровень обучения кадров (Средний: 49.4%)' } } }} />;
};
...
export const YouthStaffingNormativeChart = ({ t }) => {
    const data = {
        ...
        datasets: [
            { label: 'Штат (факт)', data: [14.5, 16.5, 28.8, 12, 67, 38, 52.2, 20, 7, 29.8, 22.5, 106.8, 30.5, 83, 38, 11, 12, 14, 28, 33], backgroundColor: '#3b82f6' },
            { label: 'Норматив', data: [44, 33, 55, 22, 121, 66, 88, 33, 11, 44, 33, 154, 44, 99, 44, 11, 12, 11, 22, 11], backgroundColor: '#e5e7eb' }
        ]
    };
...
export const YouthGenderChart = ({ t }) => {
    const data = {
        labels: ['Женщины (56.3%)', 'Мужчины (43.6%)'],
        datasets: [{
            data: [474554, 353678],
            backgroundColor: ['rgba(236, 72, 153, 0.7)', 'rgba(59, 130, 246, 0.7)'],
        }]
    };
    return <Doughnut data={data} options={{ responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Обращаемость по полу (чел.)' } } }} />;
};
...
export const YouthSpecialistStructureChart = ({ t }) => {
    const data = {
        ...
        datasets: [{
            label: 'Кол-во обращений (тыс.)',
            data: [254, 151, 141, 70, 64, 53, 41, 24, 22, 4],
            backgroundColor: 'rgba(16, 185, 129, 0.7)'
        }]
    };
    return <Bar data={data} options={{ responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Доминирование приемов психолога' } } }} />;
};
...
export const YouthMediaActivityChart = ({ t }) => {
    ...
    return <Bar data={data} options={{ responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Активность в традиционных СМИ' } } }} />;
};
...
export const YouthVideoRotationChart = ({ t }) => {
    const data = {
        ...
        datasets: [{
            label: 'Ротации / Публикации',
            data: [73762, 21163, 2189, 21819],
            backgroundColor: 'rgba(14, 165, 233, 0.7)',
        }]
    };
    return <Bar data={data} options={{ indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Трансляция видео и интернет-публикации' } } }} />;
};
...
export const YouthIomDistributionChart = ({ t }) => {
    ...
    return <Pie data={data} options={{ responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Распределение ИОМ (Всего: 298 442)' } } }} />;
};
...
export const YouthTargetReachChart = ({ t }) => {
    ...
    return <Bar data={data} options={{ indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { title: { display: true, text: 'Охват целевой аудитории (Всего: 699 679)' } } }} />;
};
